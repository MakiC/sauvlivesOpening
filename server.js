...
const connectToDatabase = require('./db'); // Ensure this path is correct
...
const startServer = async () => {
  ...
  // Endpoint to update a specific patient's diagnostic
  app.put('/api/patient/:id', (req, res) => {
    const patientId = req.params.id;
    const { diagnostic } = req.body;

    const query = 'UPDATE patient SET diagnostic = ? WHERE user_id = ?';
    db.query(query, [diagnostic, patientId], (err, result) => {
      if (err) {
        console.error('Error updating diagnostic:', err);
        return res.status(500).send('Error updating diagnostic.');
      }
      res.send('Diagnostic updated.');
    });
  });
